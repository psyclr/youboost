...
> СТАТУС (2026-06-19): ГОТОВО — по решению владельца секция-калькулятор (ServiceTiers) перенесена сразу под hero ("калькулятор сразу под херо"); доп. секции оставлены; тёмная тема единая (не вставлять белый блок). Контент data-driven, редактируется в админке.
