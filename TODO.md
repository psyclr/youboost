...
> СТАТУС (2026-06-19): текущий `/` УЖЕ рендерит весь макет (hero с метриками
> 1min/$4+/24/7/Auto + «Instant results without registration» с табами/карточками/
> корзиной — совпадает с присланным). НО на странице есть и доп. секции, которых на
> макете нет: «Why is it worth working with us», отдельный 3D-плей блок, «Three
> Steps», FAQ, footer-CTA. РАЗВИЛКА: макет = «только верх» (тогда работы нет) ИЛИ
> «сделать лендинг лаконичным» (убрать доп. секции). Нужно решение владельца.
> Контент лендинга data-driven (редактируется в админке), дизайн уже готов.
...
> СТАТУС (2026-06-19): базовый счётчик ПОДКЛЮЧЁН — `src/components/analytics/
> yandex-metrika.tsx` через next/script (afterInteractive) + noscript, только в
> production. ОСТАЛОСЬ: ecommerce-события в dataLayer (add-to-cart / purchase),
> иначе e-commerce отчёты пустые.
...
> СТАТУС (2026-06-19): по ходу работ уже поправлено — dev-команда бэка
> (LOGIN_RATE_LIMIT_MAX), E2E-раздел (три уровня тестов + изолированный стек),
> строка про CI (теперь «пайплайн строится + есть Docker e2e-стек»). ОСТАЛОСЬ:
> полный проход через claude-md-improver по пунктам ниже.
